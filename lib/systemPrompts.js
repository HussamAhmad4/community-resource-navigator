...
export function buildCunyPrompt() {
  ...
  return `You are "Navi," helping CUNY students find programs, financial aid, and support services across all CUNY campuses.
...
export function buildStudentOpportunitiesPrompt() {
  const oppCatalog = resources
    ...
    .map((r) => ({ name: r.name, shortDescription: r.shortDescription, whoItsFor: r.whoItsFor, howToApply: r.howToApply, link: r.link }))
  return `You are "Navi," helping college students find paid internships, research programs, fellowships, and scholarships.
